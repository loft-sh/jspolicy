...
func GetOriginalConfiguration(obj runtime.Object) ([]byte, error) {
	...
	original, ok := annots[applyAnnotation]
	if !ok {
		return nil, nil
	}
...
func setOriginalConfiguration(obj runtime.Object, original []byte) error {
	...
	annots[applyAnnotation] = string(original)
...
func GetModifiedConfiguration(obj runtime.Object, annotate bool, codec runtime.Encoder) ([]byte, error) {
	...
	original := annots[applyAnnotation]
	delete(annots, applyAnnotation)
	...
	if annotate {
		annots[applyAnnotation] = string(modified)
		if err := metadataAccessor.SetAnnotations(obj, annots); err != nil {
			return nil, err
		}
	...
	annots[applyAnnotation] = original
	if err := metadataAccessor.SetAnnotations(obj, annots); err != nil {
		return nil, err
	}
